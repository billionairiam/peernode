...
func (s *service) Start(ctx context.Context, r *taskAPI.StartRequest) (*taskAPI.StartResponse, error) {
	...
	s.lifecycleMu.Lock()
	// r.ExecID: 如果为空，表示启动容器的主进程。如果不为空，表示启动容器内的附加进程（例如 docker exec 命令创建的进程）。
	if r.ExecID == "" {
		cinit = container
	} else {
		// 检查容器的主进程是否已经退出
	...
	// 这是一个预启动钩子，用于执行一些前置操作（例如，为 runc 准备参数），并返回两个函数：
	// handleStarted: 用于处理进程启动后的后续逻辑。
	// cleanup: 用于在函数退出时（无论成功失败）进行清理。defer cleanup() 确保它总是会被执行。
