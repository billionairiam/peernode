...
// 这个函数的核心任务是：为一个容器的控制台（TTY）设置 I/O 重定向。
// 它从一个叫 stdin 的地方读取输入，写入控制台。
// 它从控制台读取输出，写入一个叫 stdout 的地方。
// 它使用 epoll (通过 p.epoller) 来高效地处理控制台的 I/O 事件。
// 它使用 goroutine 和 sync.WaitGroup 来并发地、异步地执行这些 I/O 拷贝操作。
func (p *linuxPlatform) CopyConsole(ctx context.Context, console console.Console, id, stdin, stdout, stderr string, wg *sync.WaitGroup) (cons console.Console, retErr error) {
	...
	if stdin != "" {
		...
		// 确保在主流程（CopyConsole 函数）返回之前，新启动的 goroutine 已经开始执行
		cwg.Add(1)
		go func() {
			cwg.Done()
			bp := bufPool.Get().(*[]byte)
