...
// publisher cdshim.Publisher: 这是一个事件发布器。Shim 在运行过程中会产生各种事件
// （如容器退出、OOM等），它通过这个 publisher 将事件发送回 containerd。这是一个回调接口。
// shutdown func(): 这是一个回调函数。当 shim 内部逻辑决定需要关闭自身时
// （例如，所有容器都已退出），它会调用这个 shutdown 函数来通知上层（启动它的进程）可以清理资源了。这是一种控制反转的设计。
func New(ctx context.Context, id string, publisher cdshim.Publisher, shutdown func()) (cdshim.Shim, error) {
	...
	// 启动一个 goroutine。这个 goroutine 的任务很可能是循环监听 s.ec (退出通道)。
	// 每当一个容器退出时，它就会从通道中收到一个 exit 消息，然后执行相应的清理逻辑，并通过 publisher 通知 containerd。
	go s.processExits()

	forwarder := s.newEventsForwarder(ctx, publisher)
	// 启动另一个 goroutine。它首先创建了一个 eventsForwarder（事件转发器）。
	// 这个转发器的任务是循环监听 s.events (通用事件通道)，
	// 并将收到的事件通过 publisher 发送给 containerd。这种设计将事件产生和事件发送解耦了。
